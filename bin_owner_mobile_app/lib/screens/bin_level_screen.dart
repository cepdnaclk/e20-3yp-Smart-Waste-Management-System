...
class _BinLevelScreenState extends State<BinLevelScreen> {
  ...
  Widget _buildLoadingView() {
    return const Center(
      child: Column(
        ...
        children: [
          CircularProgressIndicator(
            key: Key('loading-indicator'), // Added key
          ),
          SizedBox(height: 16),
          Text('Loading bin data...', style: TextStyle(color: Colors.white)),
        ],
  ...
  Widget _buildErrorBanner() {
    return Container(
      key: const Key('error-message'), // Added key
      margin: const EdgeInsets.only(bottom: 16),
      padding: const EdgeInsets.all(12),
      decoration: BoxDecoration(
        color: Colors.red.withOpacity(0.2),
  ...
  AppBar _buildAppBar() {
    return AppBar(
      ...
      actions: [
        IconButton(
          key: const Key('refresh-button'), // Added key
          icon: const Icon(Icons.refresh),
          onPressed: _fetchBins,
          tooltip: 'Refresh bin data',
        ),
            ...
            builder: (context, child) {
              ...
              return Transform.scale(
                ...
                child: Opacity(
                  opacity: opacity,
                  child: _BinCard(
                    // key: Key('bin-card-${bins[index].type}'), // Added key
                    bin: bins[index],
                    isSelected: index == _selectedBinIndex,
                  ),
                ),
